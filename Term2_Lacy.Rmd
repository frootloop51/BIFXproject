...
B = fig, ax = plt.subplots()
  ax.pie(sizes1, shadow = True, explode = explode, labels = None, autopct='%1.1f%%',pctdistance=1.25, startangle=90,radius=.8)
...
C = fig, ax = plt.subplots()
  ax.pie(sizes2, shadow = True, labels = None, autopct='%1.1f%%',pctdistance=1.25, startangle=90,radius=1)
      colors=CB_color_cycle
      plt.legend(labels=labels2,loc='center right', bbox_to_anchor=(1.25, .5)
, fontsize="8")
plt.title('What Grade Are you In?', size=10, weight="bold") 
  plt.show(C)
```
> *This graph has a number of issues. One, the colors are too similar to distinguish between the groups, two, the percentages add up to more than 100% which makes zero sense (should always be a portion of 100%). And three, the proportions for the graph are wrong given that they are all close to being equal.*
...
> *I fixed this by creating a horizontal bar chart that lists the schools by the number of projected wins in descending order. I did not add a colorblind palette, but that is another addition one could make to improve the graph further.*

## Graph 12

> *The original graph is misleading because it shows three different speed gauges that are mathematically similar, but the gauges are not proportional/do not have any scaling/units to provide context per gauge. All three values, 29, 619, 31, 786, and 26, 876 are fairly close to one another, but the middle gauge for Microsoft Edge (31,786) is maxed out/completely full where as Google Chrome (29,619) is only two-thirds full but is only 2,000 less than Microsoft Edge. Clearly, these graphs are not proportional and all graphs should have the same scale such as all of them topping out at 45,000.*
...
## Graph 15
> *This graph suffers from a lack of color and laying issues. Because the graph is in black and white, it is almost impossible to tell the difference between the different lines of the graph. Another issue is that it looks like the graph was layered over the legend, just making the graph look poorly constructed.I would print this in color and double check that the graph is layered correctly to avoid any obstructions of data or lettering.*
