...
```
```{r}
ggplot(PopData, aes(x = Year, y = CrimeRate, color = County)) +
  geom_line() +
  labs(title = "Crime Rate per 100k by County from 2010 to 2020",
       x = "Year",
...
ggplot(MD_Crime_Assault, aes(x = County, y = Violent100, fill = County)) +
  geom_boxplot() +
  labs(title = "Violent Crimesper 100k Residents by County from 2010 to 2020",
       x = "County",
...
```
```{r}
ggplot(filter(PopData_long, County %in% c("Harford County","Howard County","Kent County", "Montgomery County", "Prince George's County","Queen Anne's County
                                          ")), aes(x = Year, y = Rates)) +
  geom_line(aes(color = Type)) +
  facet_wrap(County ~ .)+
  labs(
